...
std::string sha256(const std::string &input)
{
	...
	#pragma omp parallel for num_threads(thread_count) schedule(static, 1) // <- openmp actually makes the program slower.
    for (int i = 0; i < SHA256::DIGEST_SIZE; ++i) {
		// bit shifting magic thanks to 
		// https://codereview.stackexchange.com/a/78539
		buf[i * 2] = hexmap[(digest[i] & 0xF0) >> 4];
		buf[i * 2 + 1] = hexmap[digest[i] & 0x0F];
	}

	// this also works
	/* #pragma omp parallel for num_threads(thread_count) schedule(static, 32)
	for (int i = 0; i < SHA256::DIGEST_SIZE; ++i) {
